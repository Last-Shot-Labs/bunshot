...
Features include session management (list/revoke), refresh tokens (short-lived access + long-lived refresh with rotation), MFA/TOTP (Google Authenticator, recovery codes), account deletion (immediate or queued with grace period), custom auth adapters, rate limiting on all auth endpoints, bot protection (fingerprint rate limiting + CIDR blocklist), and password set/reset flows.
