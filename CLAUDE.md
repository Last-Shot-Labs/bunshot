...
| `mfaChallenge.ts` | MFA challenge token CRUD — `createMfaChallenge`, `consumeMfaChallenge`, `replaceMfaChallengeOtp`; 4-backend (redis/mongo/sqlite/memory); configurable TTL; stores `emailOtpHash`, `createdAt`, `resendCount` for email OTP support |
...
- `mfa.ts` — MFA routes (when `auth.mfa` configured): `POST /auth/mfa/setup`, `POST /auth/mfa/verify-setup`, `POST /auth/mfa/verify`, `DELETE /auth/mfa`, `POST /auth/mfa/recovery-codes`, `GET /auth/mfa/methods`, `POST /auth/mfa/email-otp/enable`, `POST /auth/mfa/email-otp/verify-setup`, `DELETE /auth/mfa/email-otp`, `POST /auth/mfa/resend`
