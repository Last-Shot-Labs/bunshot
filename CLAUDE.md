...
| `oauth.ts` | OAuth provider coordination via `arctic` (Google, Apple, Microsoft Entra ID, GitHub) — state store set via `db.oauthState` |
...
- `requireMfaSetup` — Blocks authenticated users who have not completed MFA setup; returns `403 { code: "MFA_SETUP_REQUIRED" }`; auto-applied globally when `mfa.required: true`, also exported for per-route use; exempt paths: `/auth/*`, `/health`, `/docs`, `/openapi.json`, root `/`
