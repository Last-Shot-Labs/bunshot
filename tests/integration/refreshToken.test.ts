...
describe("refresh token basics", () => {
  ...
  test("login returns refreshToken", async () => {
    await app.request("/auth/register", json({ email: "rtlogin@example.com", password: "password123" }));

    const res = await app.request("/auth/login", json({ email: "rtlogin@example.com", password: "password123" }));
...
describe("refresh token rotation", () => {
  test("refresh returns new tokens", async () => {
    const regRes = await app.request("/auth/register", json({ email: "rot@example.com", password: "password123" }));
  ...
  test("old token works within grace window", async () => {
    const regRes = await app.request("/auth/register", json({ email: "grace@example.com", password: "password123" }));
    const { refreshToken: rt1 } = await regRes.json();
    ...
    // Should return the current token, not rotate again
    expect(body.refreshToken).toBe(rt2);
  });
});
...
describe("theft detection", () => {
  test("old token rejected after grace window", async () => {
    const regRes = await app.request("/auth/register", json({ email: "theft@example.com", password: "password123" }));
    const { refreshToken: rt1 } = await regRes.json();
  ...
  test("session destroyed after theft detection", async () => {
    const regRes = await app.request("/auth/register", json({ email: "destroy@example.com", password: "password123" }));
    const { refreshToken: rt1 } = await regRes.json();
