...
describe("SQLite MFA (TOTP)", () => {
  test("setup MFA returns secret and URI", async () => {
    const reg = await app.request("/auth/register", json({ email: "mfa@example.com", password: "password123" }));
  ...
  test("verify setup enables MFA", async () => {
    const reg = await app.request("/auth/register", json({ email: "mfa2@example.com", password: "password123" }));
    const { token } = await reg.json();

    const setupRes = await app.request("/auth/mfa/setup", { method: "POST", headers: authHeader(token) });
  ...
  test("login returns mfaRequired when MFA enabled", async () => {
    const reg = await app.request("/auth/register", json({ email: "mfa3@example.com", password: "password123" }));
    const { token } = await reg.json();
    ...
    const loginRes = await app.request("/auth/login", json({ email: "mfa3@example.com", password: "password123" }));
  ...
  test("MFA verify with TOTP completes login", async () => {
    const reg = await app.request("/auth/register", json({ email: "mfa4@example.com", password: "password123" }));
    const { token } = await reg.json();
    ...
    const loginRes = await app.request("/auth/login", json({ email: "mfa4@example.com", password: "password123" }));
  ...
  test("disable MFA with TOTP code", async () => {
    const reg = await app.request("/auth/register", json({ email: "mfa5@example.com", password: "password123" }));
    const { token } = await reg.json();
    ...
    const loginRes = await app.request("/auth/login", json({ email: "mfa5@example.com", password: "password123" }));
    const body = await loginRes.json();
