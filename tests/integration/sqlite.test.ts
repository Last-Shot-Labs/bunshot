...
beforeEach(() => {
  ...
  db.run("DELETE FROM oauth_states");
  db.run("DELETE FROM oauth_codes");
...
describe("SQLite adapter", () => {
  ...
  test("multiple logins create separate sessions", async () => {
    await app.request("/auth/register", json({ email: "sqmulti@example.com", password: "password123" }));
    const login1 = await app.request("/auth/login", json({ email: "sqmulti@example.com", password: "password123" }));
    const { token: t1 } = await login1.json();
    const login2 = await app.request("/auth/login", json({ email: "sqmulti@example.com", password: "password123" }));
  ...
  test("delete session by sessionId", async () => {
    const regRes = await app.request("/auth/register", json({ email: "sqdel@example.com", password: "password123" }));
    const { token } = await regRes.json();

    // Create a second session
    const login2 = await app.request("/auth/login", json({ email: "sqdel@example.com", password: "password123" }));
    const { token: t2 } = await login2.json();
  ...
  test("GET /auth/me returns user info", async () => {
    const regRes = await app.request("/auth/register", json({ email: "sqme@example.com", password: "password123" }));
    const { token } = await regRes.json();
  ...
  test("duplicate email registration returns 409", async () => {
    await app.request("/auth/register", json({ email: "sqdup@example.com", password: "password123" }));
    const res = await app.request("/auth/register", json({ email: "sqdup@example.com", password: "password123" }));
    expect(res.status).toBe(409);
  });

  test("set-password changes password", async () => {
    const regRes = await app.request("/auth/register", json({ email: "sqpw@example.com", password: "password123" }));
    const { token } = await regRes.json();
    ...
    const loginRes = await app.request("/auth/login", json({ email: "sqpw@example.com", password: "newpassword456" }));
    expect(loginRes.status).toBe(200);
  });

  test("DELETE /auth/me with password deletes account", async () => {
    const regRes = await app.request("/auth/register", json({ email: "sqdel2@example.com", password: "password123" }));
    const { token } = await regRes.json();
    ...
    const loginRes = await app.request("/auth/login", json({ email: "sqdel2@example.com", password: "password123" }));
    expect(loginRes.status).toBe(401);
  });

  test("DELETE /auth/me without password returns 400", async () => {
    const regRes = await app.request("/auth/register", json({ email: "sqdel3@example.com", password: "password123" }));
    const { token } = await regRes.json();
