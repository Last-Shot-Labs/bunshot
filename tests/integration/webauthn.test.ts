...
describe("WebAuthn routes", () => {
  ...
  test("DELETE /auth/mfa/webauthn/credentials/:id removes credential", async () => {
    const { token } = await registerUser("wa-rt-delcred@example.com");
  ...
  test("DELETE /auth/mfa/webauthn disables all WebAuthn", async () => {
    const { token } = await registerUser("wa-rt-disableall@example.com");
  ...
  test("login returns mfaRequired with webauthnOptions when WebAuthn enabled", async () => {
    ...
    const loginRes = await app.request("/auth/login", json({ email: "wa-rt-login@example.com", password: "password123" }));
  ...
  test("MFA verify with webauthnResponse completes login", async () => {
    ...
    // Login → get mfaToken
    const loginRes = await app.request("/auth/login", json({ email: "wa-rt-mfaverify@example.com", password: "password123" }));
