...
describe("maxSessions eviction", () => {
  ...
  test("evicts oldest session when exceeding maxSessions", async () => {
    ...
    const regRes = await evictionApp.request("/auth/register", json({ email: "evict@example.com", password: "password123" }));
    const { token: token1 } = await regRes.json();

    // Login creates session 2
    await evictionApp.request("/auth/login", json({ email: "evict@example.com", password: "password123" }));
