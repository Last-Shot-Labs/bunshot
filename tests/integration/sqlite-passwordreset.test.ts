...
describe("SQLite password reset", () => {
  ...
  test("reset password with valid token works", async () => {
    await app.request("/auth/register", json({ email: "pr2@example.com", password: "password123" }));
    await app.request("/auth/forgot-password", json({ email: "pr2@example.com" }));
    const token = captured[0].token;

    const res = await app.request("/auth/reset-password", json({ token, password: "newpassword456" }));
    expect(res.status).toBe(200);
  });

  test("login with new password succeeds", async () => {
    await app.request("/auth/register", json({ email: "pr3@example.com", password: "password123" }));
    await app.request("/auth/forgot-password", json({ email: "pr3@example.com" }));
    await app.request("/auth/reset-password", json({ token: captured[0].token, password: "newpassword456" }));

    const res = await app.request("/auth/login", json({ email: "pr3@example.com", password: "newpassword456" }));
    expect(res.status).toBe(200);
  ...
  test("reset with already-used token fails", async () => {
    await app.request("/auth/register", json({ email: "pr4@example.com", password: "password123" }));
    ...
    const res = await app.request("/auth/reset-password", json({ token, password: "anotherpassword" }));
