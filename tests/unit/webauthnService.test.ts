...
describe("generateWebAuthnAuthenticationOptions", () => {
  ...
  test("returns challenge+options when user has credentials", async () => {
    ...
    const { registrationToken } = await initiateWebAuthnRegistration(userId);
    await completeWebAuthnRegistration(userId, registrationToken, {
      id: "cred-for-auth",
...
describe("initiateWebAuthnRegistration", () => {
  test("returns options and registrationToken", async () => {
    ...
    const result = await initiateWebAuthnRegistration(userId);
    expect(result.options).toBeDefined();
...
describe("completeWebAuthnRegistration", () => {
  test("stores credential on verification and returns recovery codes", async () => {
    ...
    const result = await completeWebAuthnRegistration(userId, registrationToken, {
      id: "cred-complete",
  ...
  test("throws on invalid/expired registration token", async () => {
    ...
    await expect(
      completeWebAuthnRegistration(userId, "invalid-token", {
        id: "cred-bad",
        response: {},
      })
  ...
  test("throws on failed verification", async () => {
    ...
    await expect(
      completeWebAuthnRegistration(userId, registrationToken, {
        id: "cred-fail",
        response: {},
      })
...
describe("removeWebAuthnCredential", () => {
  test("removes credential", async () => {
    ...
    const { registrationToken: t1 } = await initiateWebAuthnRegistration(userId);
    await completeWebAuthnRegistration(userId, t1, {
      id: "cred-remove-1",
      response: {},
    });
    ...
    const { registrationToken: t2 } = await initiateWebAuthnRegistration(userId);
    await completeWebAuthnRegistration(userId, t2, {
      id: "cred-remove-2",
      response: {},
    });

    // Remove one — should succeed without identity verification
    await removeWebAuthnCredential(userId, "credential-id-123", {});
  });

  test("throws 404 for unknown credential", async () => {
    const { userId } = await registerUser("wa-remove404@example.com");
    await expect(
      removeWebAuthnCredential(userId, "nonexistent", {})
  ...
  test("requires identity verification for last credential", async () => {
    ...
    await expect(
      removeWebAuthnCredential(userId, "credential-id-123", {})
  ...
  test("removes last credential with password and disables MFA", async () => {
    ...
    await removeWebAuthnCredential(userId, "credential-id-123", { password: "password123" });
  });
});
...
describe("disableWebAuthn", () => {
  test("removes all credentials and disables MFA with password", async () => {
    ...
    await disableWebAuthn(userId, { password: "password123" });
  ...
  test("requires password verification", async () => {
    ...
    await expect(disableWebAuthn(userId, {})).rejects.toThrow("Password required");
  });
});
