...
describe("buildFingerprint", () => {
  test("returns a 12-char hex string", async () => {
    const req = new Request("http://localhost", {
      headers: { "user-agent": "Mozilla/5.0 Test Browser", accept: "text/html", "accept-language": "en-US" },
    });
  ...
  test("different User-Agent produces different fingerprint", async () => {
    const fp1 = await buildFingerprint(new Request("http://localhost", { headers: { "user-agent": "BrowserA" } }));
    const fp2 = await buildFingerprint(new Request("http://localhost", { headers: { "user-agent": "BrowserB" } }));
