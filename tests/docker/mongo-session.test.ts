...
describe("Mongo session store", () => {
  ...
  describe("persist metadata mode", () => {
    ...
    it("includes inactive sessions when configured", async () => {
      ...
      await createSession("user-inactive", "t1", "sess-inactive");
      await deleteSession("sess-inactive");

      const sessions = await getUserSessions("user-inactive");
