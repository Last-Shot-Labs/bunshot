...
import { connectTestRedis, connectTestMongo, disconnectTestServices } from "../setup-docker";
...
describe("MongoDB connection", () => {
  ...
  it("connects auth and app connections", async () => {
    ...
    const { authConnection, appConnection } = await import("../../src/lib/mongo");
    expect(authConnection.readyState).toBe(1); // connected
    expect(appConnection.readyState).toBe(1);
  });

  it("can perform basic operations", async () => {
    const { authConnection } = await import("../../src/lib/mongo");
    const db = authConnection.db!;
