...
import { mongoAuthAdapter } from "../../src/adapters/mongoAuth";
...
describe("mongoAuthAdapter", () => {
  ...
  describe("findOrCreateByProvider", () => {
    it("creates a new user for unknown provider", async () => {
      const result = await mongoAuthAdapter.findOrCreateByProvider!(
        "google", "gid-123", { email: "oauth@example.com" }
      );
    ...
    it("finds existing user by provider key", async () => {
      const first = await mongoAuthAdapter.findOrCreateByProvider!(
        "google", "gid-456", { email: "existing@example.com" }
      );
      const second = await mongoAuthAdapter.findOrCreateByProvider!(
        "google", "gid-456", { email: "existing@example.com" }
      );
